...
param(
  ...
  [Parameter(Mandatory=$false)][string[]]$OrchProviders = @("openai","openai")
)
...
function Get-Array([object]$obj,[string]$name){ To-Array (Get-Prop $obj $name) }
...
function Get-PromoteResultSafe([object]$resp){ Get-DerivedValue $resp "promote_result" }
...
function Normalize-AttemptsArray([object]$attempts){
  if($null -eq $attempts){ return @() }
  if(($attempts -is [System.Collections.IEnumerable]) -and -not ($attempts -is [string])){ return @($attempts) }
  $attemptNo = Get-Prop $attempts "attempt_no"
  if($null -eq $attemptNo){ $attemptNo = Get-Prop $attempts "attemptNo" }
  if($null -ne $attemptNo){ return @($attempts) }
  return @()
}

function Attempts-ErrorRate([object]$resp){
  $aa = Get-DerivedValue $resp "adapter_attempts"
  $runs = To-Array $aa
  $total = 0
  $err = 0
  foreach($r in $runs){
    $arr = Normalize-AttemptsArray (Get-Prop $r "attempts")
    foreach($a in $arr){
      $total++
      $st = [string](Get-Prop $a "status")
      $out = [string](Get-Prop $a "outcome")
      if($st -ne "success" -and $out -ne "success"){ $err++ }
    }
  }
  if($total -le 0){ return 0.0 }
  return ([double]$err / [double]$total)
}

function Chosen-Success([object]$resp){
  $meta = Get-Prop $resp "meta"
  $chosenProv = [string](Get-Prop $meta "provider")
  $chosenModel = [string](Get-Prop $meta "model")
  ...
  foreach($r in $runs){
    ...
    if($p -eq $chosenProv -and $m -eq $chosenModel){
      $arr = Normalize-AttemptsArray (Get-Prop $r "attempts")
      foreach($a in $arr){
        $st = [string](Get-Prop $a "status")
        $out = [string](Get-Prop $a "outcome")
        if($st -eq "success" -or $out -eq "success"){ return 1.0 }
      }
      return 0.0
    }
  }
...
function Chosen-LatencyMs([object]$resp){
  $meta = Get-Prop $resp "meta"
  $chosenProv = [string](Get-Prop $meta "provider")
  $chosenModel = [string](Get-Prop $meta "model")
  ...
  foreach($r in $runs){
    ...
    if($p -eq $chosenProv -and $m -eq $chosenModel){
      $arr = Normalize-AttemptsArray (Get-Prop $r "attempts")
      if($arr.Count -ge 1){
        $lat = Get-Prop $arr[0] "latency_ms"
        if($lat -is [int] -or $lat -is [long] -or $lat -is [double]){ return [double]$lat }
      }
      return 0.0
    }
  }
...
function Conflict-Rate([object]$resp){
  $conf = (Get-Array $resp "conflicts")
  $claims = (Get-Array $resp "claims")
  $den = [double]([Math]::Max(1, @($claims).Count))
  return ([double]@($conf).Count / $den)
}

if ($ProjectId.Trim().Length -eq 0) { throw "ProjectId is empty." }
if ($BaseUrl.Trim().Length -eq 0) { throw "BaseUrl is empty." }
if ($SingleProvider.Trim().Length -eq 0) { throw "SingleProvider is empty." }
if ($null -eq $OrchProviders -or $OrchProviders.Count -lt 1) { throw "OrchProviders must have at least 1 entry." }
...
$sumSingleConflictRate = 0.0
$sumOrchConflictRate = 0.0
$sumSingleAttemptsErrRate = 0.0
$sumOrchAttemptsErrRate = 0.0
$sumSingleChosenSuccess = 0.0
$sumOrchChosenSuccess = 0.0
$sumSingleLatency = 0.0
$sumOrchLatency = 0.0
...
foreach($c in $cases){
  ...
  $orchResp   = Invoke-Chat -baseUrl $BaseUrl -projectId $ProjectId -threadId $OrchThreadId   -mode "orchestra" -prompt $prompt -extra $extra -providers @($OrchProviders) -promote ([bool]$EnablePromote)
  ...
  $sumDelta += $deltaRealSum
  $sumOrchRate += $orchRealRate
  $sumSingleRate += $singleRealRate

  $sc = Conflict-Rate $singleResp
  $oc = Conflict-Rate $orchResp
  $se = Attempts-ErrorRate $singleResp
  $oe = Attempts-ErrorRate $orchResp
  $ss = Chosen-Success $singleResp
  $os = Chosen-Success $orchResp
  $sl = Chosen-LatencyMs $singleResp
  $ol = Chosen-LatencyMs $orchResp

  $sumSingleConflictRate += $sc
  $sumOrchConflictRate += $oc
  $sumSingleAttemptsErrRate += $se
  $sumOrchAttemptsErrRate += $oe
  $sumSingleChosenSuccess += $ss
  $sumOrchChosenSuccess += $os
  $sumSingleLatency += $sl
  $sumOrchLatency += $ol
  ...
  $row = [ordered]@{
    ...
    single = [ordered]@{
      ...
      conflict_rate = $sc
      attempts_error_rate = $se
      chosen_success = $ss
      chosen_latency_ms = $sl
    ...
    orchestra = [ordered]@{
      providers = @($OrchProviders)
      evidence = $orchCls
      evidence_ids_total = [int]$orchCls["provenance"]["total"]
      conflict_rate = $oc
      attempts_error_rate = $oe
      chosen_success = $os
      chosen_latency_ms = $ol
...
$avgSingleConflict = 0.0
$avgOrchConflict = 0.0
$avgSingleAttemptsErr = 0.0
$avgOrchAttemptsErr = 0.0
$singleChosenSuccessRate = 0.0
$orchChosenSuccessRate = 0.0
$avgSingleLatency = 0.0
$avgOrchLatency = 0.0
...
if($total -gt 0){
  ...
  $avgSingleConflict = $sumSingleConflictRate / [double]$total
  $avgOrchConflict = $sumOrchConflictRate / [double]$total
  $avgSingleAttemptsErr = $sumSingleAttemptsErrRate / [double]$total
  $avgOrchAttemptsErr = $sumOrchAttemptsErrRate / [double]$total
  $singleChosenSuccessRate = $sumSingleChosenSuccess / [double]$total
  $orchChosenSuccessRate = $sumOrchChosenSuccess / [double]$total
  $avgSingleLatency = $sumSingleLatency / [double]$total
  $avgOrchLatency = $sumOrchLatency / [double]$total
}
...
$summary = [ordered]@{
  ...
  orchestra_providers = @($OrchProviders)

  avg_single_conflict_rate = $avgSingleConflict
  avg_orch_conflict_rate = $avgOrchConflict
  avg_single_attempts_error_rate = $avgSingleAttemptsErr
  avg_orch_attempts_error_rate = $avgOrchAttemptsErr
  single_chosen_success_rate = $singleChosenSuccessRate
  orch_chosen_success_rate = $orchChosenSuccessRate
  avg_single_chosen_latency_ms = $avgSingleLatency
  avg_orch_chosen_latency_ms = $avgOrchLatency
}
