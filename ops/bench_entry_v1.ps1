param(
  ...
  [int[]]$TailCandidates = @(2000, 10000, 50000, 200000, 400000)
)
...
function Find-FinalizedLineByMetaMarkerAndCaseId {
  ...
  foreach ($n in $TailCandidates) {
    ...
    if (($tail -like "*$needle1*") -and ($tail -like "*$needle2*")) {
      ...
      foreach ($ln in ($lines | Select-Object -Last 6000)) {
...
foreach ($c in $cases) {
  ...
  $body = [ordered]@{}
  foreach ($p in $c.PSObject.Properties) { $body[$p.Name] = $p.Value }
  $body["meta"] = $meta
  $body["projectId"] = $ProjectId
  $body["threadId"] = $ThreadId

  $url = ($Base.TrimEnd("/") + "/api/chat")
  Invoke-WebJson -Url $url -Method "POST" -BodyObj $body -TimeoutSec 90 | Out-Null
}
...
foreach ($c in $cases) {
  $caseId = $c.caseId.ToString()
  $hit = Find-FinalizedLineByMetaMarkerAndCaseId -FinalizedPath $finalizedPath -MarkerKey $MarkerKey -BenchRunId $benchRunId -CaseId $caseId -TailCandidates $TailCandidates
  if (-not $hit.found) {
    $map[$caseId] = $null
    $countsMap[$caseId] = $null
    continue
  }
...
foreach ($c in $cases) {
  ...
  if ($t.has -and $null -ne $t.value) {
    $confChecks++
    if ($got -ne $t.value) {
      $confMismatches += [pscustomobject]@{ caseId=$caseId; exp=$t.value; got=$got }
    }
  }
}
