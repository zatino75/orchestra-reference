...
function Invoke-WriteAtomicUtf8 {
  ...
  $full = $PathLike
  if (-not [System.IO.Path]::IsPathRooted($full)) { $full = Join-Path $RepoRoot $PathLike }
  $full = (Resolve-Path -LiteralPath (Split-Path -Parent $full) -ErrorAction SilentlyContinue)?.Path + "\" + (Split-Path -Leaf $full)
  if ($full -like "\*" -and -not (Test-Path -LiteralPath (Split-Path -Parent $full))) {
    Ensure-Directory -DirPath (Split-Path -Parent $full)
  }
  ...
  try {
    if ($WriterInfo.has_writer -and $null -ne $WriterInfo.writer_cmd) {
      ...
      if ($params.ContainsKey("PathLike")) {
        ...
        Write-AtomicUtf8Fallback -FullPath $full -Content $Content
      }
    } else {
      Write-AtomicUtf8Fallback -FullPath $full -Content $Content
    }
  ...
  return @{ path=$full; fallback_used=$fallbackUsed; used=$used }
}
...
if ($missingCase.Count -gt 0) { $pass = $false }
if ($confMismatches.Count -gt 0) { $pass = $false }
...
# rewrite once with writer metadata filled
