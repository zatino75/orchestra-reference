param(
  ...
  [int[]]$TailCandidates = @(2000, 10000, 50000, 200000, 400000, 800000, 1200000),
...
function Find-FinalizedLineByMetaMarkerAndCaseId {
  ...
  foreach ($n in $TailCandidates) {
    ...
    if (($tail -like "*$needle1*") -and ($tail -like "*$needle2*")) {
      ...
      foreach ($ln in ($lines | Select-Object -Last 12000)) {
...
if (-not $benchPresent) {
  throw "Bench run id not found in finalized. This call path still doesn't preserve bench_run_id. bench_run_id=$benchRunId"
}
